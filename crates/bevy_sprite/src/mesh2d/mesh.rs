...
use bevy_render::render_asset::{prepare_assets, RenderAssetKey};
use bevy_render::{
    batching::{
    ...
    fn build(&self, app: &mut bevy_app::App) {
        ...
        if let Ok(render_app) = app.get_sub_app_mut(RenderApp) {
            render_app
                ...
                .init_resource::<PendingMesh2dAssetKeys>()
...
pub struct PendingMesh2dAssetKeys(Vec<(Entity, AssetId<Mesh>)>);
...
pub fn extract_mesh2d(
    ...
    mut pending_mesh2d_asset_keys: ResMut<PendingMesh2dAssetKeys>,
    ...
    for (entity, view_visibility, transform, handle, no_automatic_batching) in &query {
        ...
        let mesh_asset_key = render_meshes.get_key(handle.0.id()).unwrap_or_else(|| {
            pending_mesh2d_asset_keys.push((entity, handle.0.id()));
...
pub fn update_mesh2d_asset_keys(
    mut pending_mesh2d_asset_keys: ResMut<PendingMesh2dAssetKeys>,
    render_meshes: Res<RenderAssets<Mesh>>,
    mut render_mesh2d_instances: ResMut<RenderMesh2dInstances>,
) {
    for (entity, asset_id) in pending_mesh2d_asset_keys.drain(..) {
        let Some(mesh_asset_key) = render_meshes.get_key(asset_id) else {
            continue;
        };
        let Some(render_mesh2d_instance) = render_mesh2d_instances.get_mut(&entity) else {
            continue;
        };
        render_mesh2d_instance.mesh_asset_key = mesh_asset_key;
    }
}
