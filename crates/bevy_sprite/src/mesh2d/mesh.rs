...
use bevy_render::{
    ...
    renderer::{render_system, RenderAdapter, RenderContext, RenderDevice, RenderQueue},
    texture::{
...
use crate::{
    render, ColorMaterial, Material2dBindGroupId, RenderMaterial2dInstances, RenderMaterial2dKey,
    RenderMaterials2d,
};
...
pub fn render_from_draw_streams(
    ...
    let render_adapter = render_adapter.into_inner();
    let render_device = render_device.into_inner();
    ...
    let mut render_context = RenderContext::new(render_device.clone(), render_adapter.get_info());
        ...
        views.iter()
    {
        ...
        if has_storage_buffers {
            render_pass.set_bind_group(1, &mesh_bind_group.value, &[]);
        }
