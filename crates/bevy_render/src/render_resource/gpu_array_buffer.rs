use super::{DataArrayTexture, StorageBuffer};
use crate::{
...
pub trait GpuArrayBufferable: ShaderType + ShaderSize + WriteInto + Clone + Pod + Default {}
impl<T: ShaderType + ShaderSize + WriteInto + Clone + Pod + Default> GpuArrayBufferable for T {}
...
pub enum GpuArrayBuffer<T: GpuArrayBufferable> {
    ...
    DataTexture(DataArrayTexture<T>),
...
impl<T: GpuArrayBufferable> GpuArrayBuffer<T> {
    pub fn new(device: &RenderDevice) -> Self {
        ...
        if limits.max_storage_buffers_per_shader_stage == 0 {
            GpuArrayBuffer::DataTexture(DataArrayTexture::new(device))
        } else {
    ...
    pub fn clear(&mut self) {
        match self {
            GpuArrayBuffer::DataTexture(buffer) => buffer.clear(),
    ...
    pub fn push(&mut self, value: T) -> GpuArrayBufferIndex<T> {
        match self {
            GpuArrayBuffer::DataTexture(buffer) => buffer.push(value),
    ...
    pub fn write_buffer(&mut self, device: &RenderDevice, queue: &RenderQueue) {
        match self {
            GpuArrayBuffer::DataTexture(buffer) => buffer.write_buffer(device, queue),
    ...
    pub fn binding_layout(
        ...
        if device.limits().max_storage_buffers_per_shader_stage == 0 {
            DataArrayTexture::<T>::binding_layout(binding, visibility, device)
        } else {
            BindGroupLayoutEntry {
                binding,
                visibility,
    ...
    pub fn binding(&self) -> Option<BindingResource> {
        match self {
            GpuArrayBuffer::DataTexture(buffer) => buffer.binding(),
