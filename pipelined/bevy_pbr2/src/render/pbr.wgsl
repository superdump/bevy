...
let FLAGS_AMBIENT_OCCLUSION_TEXTURE_BIT: u32  = 64u;
...
[[group(0), binding(6)]]
var ambient_occlusion_texture: texture_2d<f32>;
[[group(0), binding(7)]]
var ambient_occlusion_sampler: sampler;
...
fn fragment(in: FragmentInput) -> [[location(0)]] vec4<f32> {
    ...
    if ((material.flags & FLAGS_UNLIT_BIT) == 0u) {
        ...
        if ((material.flags & FLAGS_AMBIENT_OCCLUSION_TEXTURE_BIT) != 0u) {
            let frame_size = textureDimensions(ambient_occlusion_texture);
            let ao_uv = vec2<f32>(in.frag_coord.x / f32(frame_size.x), in.frag_coord.y / f32(frame_size.y));
            ambient_occlusion = textureSampleLevel(ambient_occlusion_texture, ambient_occlusion_sampler, ao_uv, 0.0).r;
        }
        ...
        output_color = vec4<f32>(
            light_accum +
                (diffuse_ambient + specular_ambient) * lights.ambient_color.rgb * occlusion * ambient_occlusion +
