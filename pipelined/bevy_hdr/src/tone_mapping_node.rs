...
use bevy_render2::{
    ...
    render_resource::{
        ...
        BlendOperation, BlendState, ColorTargetState, ColorWrite, Face, FragmentState, LoadOp,
        ...
        ShaderStage, TextureFormat, TextureSampleType, TextureViewDimension, TextureViewId,
        VertexState,
    },
...
impl Node for ToneMappingNode {
    ...
    fn run(
        ...
        if let Some(camera_3d) = extracted_cameras.entities.get(CameraPlugin::CAMERA_3D) {
            ...
            let swap_chain_texture = extracted_window.swap_chain_frame.as_ref().unwrap();
            ...
            render_pass.set_bind_group(0, &hdr_target_bind_group.as_ref().unwrap().value(), &[]);
