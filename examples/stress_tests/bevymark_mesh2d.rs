...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>, mut meshes: ResMut<Assets<Mesh>>) {
    ...
    commands.insert_resource(BirdAssets {
        mesh: Mesh2dHandle(meshes.add(Mesh::from(shape::Quad::default()))),
        texture,
    });
...
fn spawn_birds(
    ...
    let material = materials.add(ColorMaterial {
        color: counter.color,
        texture: Some(bird_assets.texture.clone_weak()),
    });
    for count in 0..spawn_count {
        let bird_z = (counter.count + count) as f32 * 0.00001;
        commands.spawn((
            MaterialMesh2dBundle {
                mesh: Mesh2dHandle(bird_assets.mesh.0.clone_weak()),
                material: material.clone(),
                transform: Transform {
                    translation: Vec3::new(bird_x, bird_y, bird_z),
                    scale: Vec3::splat(256.0 * BIRD_SCALE),
                    ..default()
                },
                ..default()
            },
